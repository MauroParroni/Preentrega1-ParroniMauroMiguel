const videojuegos = [
  ...
  },
  {
    ...
    categoria: "Terror",
    imagen:
      "https://www.digiseller.ru/preview/429992/p1_3618446_cb2f6be6.jpg",
...
//exportamos un solo producto
export function getProductData(idURL) {
  return new Promise((resolve, reject) => {    
    const videojuegoFind = videojuegos.find((game)=> game.id === parseInt(idURL))
    setTimeout(() => {
      resolve(videojuegoFind);
    }, 2000);
  });
}
export function getCategoryData(categoryURL){
  return new Promise((resolve, reject) => {
    const videojuegosFilter = videojuegos.filter((game)=> {
      return game.categoria.toLowerCase() === categoryURL.toLowerCase();
    })
    setTimeout(() => {
      resolve(videojuegosFilter);
    }, 2000);
})
}
